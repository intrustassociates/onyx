...
# file_metadata keys stamped onto generated artifacts. The SharePoint upload
# endpoint uses these to verify that a file_id refers to a docx generated by
# this tool AND owned by the requesting user — without them, any authenticated
# user could exfiltrate arbitrary file_store entries (IDOR).
ARTIFACT_GENERATOR_KEY = "generator"
ARTIFACT_GENERATOR_VALUE = "generate_docx"
ARTIFACT_OWNER_KEY = "owner_user_id"
ARTIFACT_SAVED_URL_KEY = "sharepoint_web_url"


def build_artifact_metadata(user_id: str | None) -> dict[str, str]:
    metadata = {ARTIFACT_GENERATOR_KEY: ARTIFACT_GENERATOR_VALUE}
    if user_id:
        metadata[ARTIFACT_OWNER_KEY] = user_id
    return metadata


def is_docx_artifact_owned_by(
    file_metadata: Any,
    file_type: str | None,
    user_id: str | None,
) -> bool:
    """True iff the file record is a generate_docx artifact accessible to ``user_id``.

    Artifacts stamped without an owner (auth-disabled deployments) are
    accessible to any authenticated user; everything else requires an exact
    owner match. Records without the generator marker are always rejected.
    """
    if file_type != DOCX_MIME:
        return False
    if not isinstance(file_metadata, dict):
        return False
    if file_metadata.get(ARTIFACT_GENERATOR_KEY) != ARTIFACT_GENERATOR_VALUE:
        return False
    owner = file_metadata.get(ARTIFACT_OWNER_KEY)
    if owner is None:
        return True
...
class GenerateDocxTool(Tool[None]):
    ...
    def __init__(
        ...
        *,
        ...
        self._id = tool_id
        # Stamped into file_metadata so the upload endpoint can enforce
        # ownership. ``None`` only in auth-disabled deployments.
        ...
        **llm_kwargs: Any,
        ...
        try:
            file_id = file_store.save_file(
                ...
                file_type=DOCX_MIME,
                file_metadata=build_artifact_metadata(self._user_id),
            )
        except Exception as exc:
